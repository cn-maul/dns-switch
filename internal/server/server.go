// Package server provides the DNS-Switch web management panel.
package server
...
async function loadServers() {
	try {
		...
		servers = d.servers || {};
		renderTable(d);
	} catch(e) { showToast('无法连接服务器', true); }
}
...
async function runTest() {
	...
	} catch(e) { showToast('测速请求失败', true); }
	finally { btn.disabled = false; btn.textContent = '📡 全部测速'; }
}
async function setDns(name) {
	try {
		const r = await fetch('/api/set?name=' + encodeURIComponent(name), { method: 'POST' });
		const d = await r.json();
		if (d.ok) { document.getElementById('currentDns').textContent = name; showToast('已切换到 ' + name); }
		else { showToast(d.error || '切换失败', true); }
...
async function restoreDns() {
	try {
		...
		const d = await r.json();
		if (d.ok) { document.getElementById('currentDns').textContent = 'DHCP'; showToast('已恢复为 DHCP'); }
...
// Server holds the state and dependencies for the HTTP management panel.
type Server struct {
...
func New(cfgStore config.Store, dnsMgr *dns.Manager) *Server {
	return &Server{
		cfgStore: cfgStore,
		dnsMgr:   dnsMgr,
...
// ListenAndServe starts the HTTP server on addr and opens the browser.
// Blocks until SIGINT or SIGTERM, then performs graceful shutdown.
func (s *Server) ListenAndServe(addr string) error {
	// Initialize currentDNS from config backup state
	if cfg, err := s.cfgStore.Read(); err == nil && cfg.Backup != nil {
		s.state.setCurrentDNS("已设置（查看备份记录）")
	} else {
		s.state.setCurrentDNS("DHCP")
	}
...
func (s *Server) handleStatus(w http.ResponseWriter, r *http.Request) {
	...
	results, bestIdx := s.state.getResults()
	resp := statusResponse{
		Servers:    cfg.Servers,
		CurrentDns: s.state.getCurrentDNS(),
...
func (s *Server) handleTest(w http.ResponseWriter, r *http.Request) {
	...
	bench.Run(cfg.Servers, func(results []bench.Result, idx int) {
		s.state.setResults(results, idx)
	})
...
func (s *serverState) getResults() ([]bench.Result, int) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.benchResults, s.bestIdx
}

func (s *serverState) setResults(results []bench.Result, idx int) {
	s.mu.Lock()
	defer s.mu.Unlock()
